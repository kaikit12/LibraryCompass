                ...
                (snapshot: any) => {
                    try {
                        const borrowals = snapshot.docs.map((d: any) => {
                            const data = d.data();
                            // Defensive: check dueDate
                            let dueDate: Date;
                            if (data.dueDate?.toDate) {
                                dueDate = data.dueDate.toDate();
                            } else {
                                console.error(`Borrowal ${d.id} missing or invalid dueDate, using today as fallback.`);
                                dueDate = new Date();
                            }
                            return {
                                id: d.id,
                        ...
                        console.error("Lỗi khi xử lý active borrowals:", err);
                    } finally {
                        setIsLoading(false);
                    }
                },
                (error: any) => {
                    console.error("Lỗi khi tải active borrowals:", error);
                    setIsLoading(false);
                }
            );
            return () => unsubscribe();
        ...
        useEffect(() => {
            ...
            const unsub = safeOnSnapshot(qHistory, async (snapshot: any) => {
                try {
                    ...
                    const items = await Promise.all(snapshot.docs.map(async (d: any) => {
                        ...
                        if (!book) {
                            try {
                                ...
                                console.error(`Không thể lấy sách ID: ${data.bookId}`, bookErr);
                                return null;
                            }
                        }
                        // Defensive for all date fields
                        const borrowedAt = data.borrowedAt?.toDate?.() || null;
                        const returnedAt = data.returnedAt?.toDate?.() || null;
                        const dueDate = data.dueDate?.toDate?.() || null;
                        let status = 'Đúng hạn';
                        if (returnedAt && dueDate && returnedAt > dueDate) {
                            status = 'Trả trễ';
                        }
                        return book ? {
                            id: d.id,
                    ...
                    console.error("Lỗi khi xử lý lịch sử mượn/trả:", err);
                    setHistory([]);
                }
            });
            return () => unsub();
